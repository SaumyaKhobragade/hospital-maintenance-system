...
export default function QueueDetails() {
  ...
  // derive capacity from SSE or fall back to patients count * estimate
  const totalCapacity = stats ? Math.max(patients.length, stats.totalHospitals * 30) : 320;
  const activeDoctors = stats?.totalDoctorsActive ?? treatments.length;
  const totalDoctors = Math.max(activeDoctors, 10);
  const idleDoctors = Math.max(0, totalDoctors - activeDoctors);
  const isAboveSafe = patients.length > totalCapacity * 0.75;

  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch("/api/db/queue");
        const json = await res.json();

        if (json.patients) {
          const mapped = json.patients.map((d: any) => ({
        ...
        if (json.treatments) {
          const mappedT = json.treatments.map((t: any) => ({
      ...
      <div className="grid grid-cols-2 gap-5">
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          ...
          <div className="flex items-baseline gap-2 mb-3">
            ...
            <span className="text-slate-500">/ {totalCapacity} capacity</span>
          </div>
          <div className="h-3 bg-slate-100 rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-blue-500 via-amber-500 to-rose-500" style={{ width: `${Math.min(100, (patients.length / totalCapacity) * 100)}%` }} />
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <div className="text-slate-900" style={{ fontSize: "32px", fontWeight: 700 }}>{activeDoctors}</div>
            </div>
            <div>
              <div className="flex items-center gap-2 text-slate-500 text-sm mb-1"><Users className="w-4 h-4" /> Idle</div>
              <div className="text-slate-900" style={{ fontSize: "32px", fontWeight: 700 }}>{idleDoctors}</div>
            </div>
          </div>
          {activeDoctors > totalDoctors * 0.8 && (
            <div className="bg-amber-50 border border-amber-100 rounded-lg px-3 py-2 text-xs text-amber-700 flex items-center gap-2">
              <AlertCircle className="w-3.5 h-3.5" /> Fatigue Warning · {Math.max(1, Math.floor(activeDoctors / 3))} doctors above 10h shift
            </div>
          )}
        </div>
      </div>
