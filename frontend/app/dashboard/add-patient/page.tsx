...
import { analyzeUploadedImage, InjuryAnalysisResult } from "../../../lib/api/image";
...
export default function AddPatient() {
  ...
  // Injury Analysis States
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [analysis, setAnalysis] = useState<InjuryAnalysisResult | null>(null);
  ...
  const handleInjuryFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error(err);
      setError("Backend connection failed. Using mock fallback.");
      // Fallback mock data
      setAnalysis({
        analysisId: "mock-id",
        severityScore: 74,
        severityLevel: "MEDIUM",
        routingRecommendation: "Doctor evaluation (Trauma · Bay 3)",
        features: {
          woundAreaRatio: 0.15,
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          <div className="flex items-center gap-2 mb-5 pb-3 border-b border-slate-50">
            ...
            <div>
              <h3 className="text-slate-900 font-semibold">Medical Documents & Visuals</h3>
              <p className="text-xs text-slate-400">Upload medical history files for ChromaDB, and run real-time visual assessment on injury photos.</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
            ...
            <div className="lg:col-span-6 flex flex-col gap-2">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">General Medical Records (Optional)</span>
              <label
                className="flex-1 min-h-[200px] border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-blue-300 hover:bg-blue-50/30 transition gap-2"
            ...
            <div className="lg:col-span-6 flex flex-col gap-2">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Injury Visual Analysis (Optional)</span>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <input
                    type="file"
                    id="injury-image-input"
                    accept="image/*"
                    onChange={handleInjuryFileChange}
                    className="hidden"
                  />
                  <label
                    htmlFor="injury-image-input"
                    className="aspect-square border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-blue-300 hover:bg-blue-50/30 transition relative overflow-hidden block animate-fade-in"
                  >
                    {preview ? (
                      <>
                        ...
                        <button type="button" onClick={(e) => { e.stopPropagation(); setBlur(!blur); }}
                          className="absolute top-3 right-3 px-2 py-1 rounded-lg bg-black/60 text-white text-[10px] flex items-center gap-1 z-10">
                          {blur ? <><EyeOff className="w-3 h-3" /> Blurred</> : <><Eye className="w-3 h-3" /> Clear</>}
                        </button>
                        <div className="absolute bottom-3 left-3 bg-black/50 text-white text-[10px] px-2 py-1 rounded-lg z-10">
                          Injury Preview
                        </div>
                      </>
                    ) : (
                      <>
                        ...
                        <span className="text-sm font-semibold text-slate-700 text-center px-2">Upload Injury Image</span>
                        <span className="text-[10px] text-slate-400">For real-time AI visual analysis</span>
                      </>
                    )}
                  </label>
                </div>

                <div className="bg-gradient-to-br from-blue-50/50 to-violet-50/50 rounded-2xl p-4 border border-blue-100/50 flex flex-col justify-between min-h-[200px]">
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      <Sparkles className="w-4 h-4 text-blue-600" />
                      <span className="text-xs font-semibold text-slate-800">AI Visual Analysis</span>
                      {loading && <span className="ml-auto text-[10px] text-blue-600 flex items-center gap-1"><Loader2 className="w-3 h-3 animate-spin" /> Analyzing...</span>}
                      {!loading && analysis && <span className="ml-auto text-[10px] text-emerald-600 font-semibold">● Complete</span>}
                    </div>

                    {error && (
                      <div className="mb-2 px-2.5 py-1.5 bg-amber-50 border border-amber-100 rounded-xl flex items-start gap-1.5 text-[10px] text-amber-800">
                    ...
                    {loading ? (
                      <div className="flex flex-col items-center justify-center py-6 text-slate-500">
                        ...
                        <p className="text-[10px] text-center max-w-[150px]">Uploading and detecting features on unified backend...</p>
                      </div>
                    ) : analysis ? (
                      <div className="space-y-2.5">
                        <Metric label="Severity Score" value={`${(analysis.severityScore / 10).toFixed(1)} / 10 (${analysis.severityLevel})`} color={analysis.severityLevel === "HIGH" ? "bg-rose-500" : analysis.severityLevel === "MEDIUM" ? "bg-amber-500" : "bg-emerald-500"} pct={analysis.severityScore} />
                        <Metric label="Confidence" value={`${Math.round(analysis.confidence * 100)}%`} color="bg-blue-500" pct={analysis.confidence * 100} />
                        
                        <div className="bg-white/80 rounded-xl p-2.5 border border-blue-100/60 text-[11px]">
                          <p className="text-slate-400 mb-0.5 font-medium">Routing Recommendation</p>
                          <p className="text-slate-800 font-semibold">{analysis.routingRecommendation}</p>
                        </div>

                        <div className="bg-white/80 rounded-xl p-2.5 border border-blue-100/60 text-[11px]">
                          <p className="text-slate-400 mb-0.5 font-medium">Description</p>
                          <p className="text-slate-600 leading-relaxed text-left">{analysis.explanation}</p>
                        </div>
                      </div>
                    ) : (
                      <div className="text-xs text-slate-400 flex flex-col items-center gap-1.5 py-8 justify-center">
                        <AlertCircle className="w-5 h-5 text-slate-300" />
                        <span>Upload injury image to analyze.</span>
                      </div>
                    )}
                  </div>
                </div>
